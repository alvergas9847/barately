...
def teclado_si_no() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup([["1 Si", "2 No"]], resize_keyboard=True)

def teclado_cancelar() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup([["0 Cancelar"]], resize_keyboard=True)
...
MSG_ERROR_GENERAL   = "Ocurrio un error. Intenta de nuevo o escribe /start."
...
def formato_venta_resumen(items: list, pagos: list, cliente_nombre: str) -> str:
    ...
    for it in items:
        ...
        msg += f"  {it['nombre']} T:{it['talla']}{color} x{it['cantidad']} = {q(it['subtotal'])}\n"
    msg += f"\n*TOTAL: {q(total)}*\n\n"
    msg += "Pagos:\n"
    for p in pagos:
...
def formato_resumen_dia(data: list) -> str:
    ...
    for r in data:
        ...
        if tv > 0:
            msg += f"*{r['vendedor']}*\n"
            msg += f"   Ventas: {r['num_ventas']} | {q(tv)}\n"
...
def formato_alertas(alertas: list) -> str:
    ...
    for a in alertas:
        tipo = ICONOS_ALERTA.get(a.get("tipo", ""), "ALERTA")
        msg += f"[{tipo}] *{a['titulo']}*\n"
        msg += f"   {a['mensaje']}\n\n"
    return msg
