...
from handlers.auth import usuario_actual, verificar_sesion, tiene_permiso
...
def _es_cancelar(texto: str) -> bool:
    return texto in ("❌ Cancelar", "🔙 Volver al menú")
...
async def buscar_stock(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for p in productos:
        ...
        if variantes:
            for v in sorted(variantes, key=lambda x: (x.get("tallas") or {}).get("nombre", "")):
                talla  = (v.get("tallas") or {}).get("nombre", "?")
                color  = (v.get("colores") or {}).get("nombre", "")
                alerta = " ⚠️" if v["stock"] <= v.get("stock_minimo", 0) else ""
                ctxt   = f" {color}" if color else ""
                msg   += f"  T:{talla}{ctxt} → *{v['stock']}* uds{alerta}\n"
        else:
...
async def vale_monto(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        ...
        await update.message.reply_text(
            "Escribe un monto válido. Ejemplo: *150*",
            parse_mode="Markdown"
        )
        return VALE_MONTO
...
async def vale_motivo(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if exito:
        await update.message.reply_text(
            ...
            f"El encargado o administrador lo revisará en breve.",
            parse_mode="Markdown",
...
async def resumen_dia(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not tiene_permiso(usuario, ["admin", "encargado"]):
        await update.message.reply_text(
            "⛔ No tienes permiso para ver el resumen.",
            reply_markup=teclado_menu(usuario["rol"])
        )
        return 1
...
async def ver_alertas(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not tiene_permiso(usuario, ["admin", "encargado"]):
        await update.message.reply_text(
            "⛔ No tienes permiso para ver las alertas.",
            reply_markup=teclado_menu(usuario["rol"])
        )
        return 1
